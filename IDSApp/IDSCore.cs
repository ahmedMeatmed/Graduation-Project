...
namespace IDSApp
{
    ...
    public class IDSCore : IDisposable
    {
     ...
     string info)
        ...
        private int MapFlowDirection(string direction)
        {
        ...
        private int MapTcpFlags(string flags)
        {
            int result = 0;
            ...
            foreach (var f in parts)
            {
                switch (f.ToUpper())
                {
                    case "FIN": result |= 1 << 0; break;   // 1
                    case "SYN": result |= 1 << 1; break;   // 2
                    case "RST": result |= 1 << 2; break;   // 4
                    case "PSH": result |= 1 << 3; break;   // 8
                    case "ACK": result |= 1 << 4; break;   // 16
                    case "URG": result |= 1 << 5; break;   // 32
                    case "ECE": result |= 1 << 6; break;   // 64
                    case "CWR": result |= 1 << 7; break;   // 128
                }
            }
            return result;
        }
        ...
        private async Task ProcessPacketData(IPPacket ipPacket, byte[] payload, string srcIp, string dstIp,
                                          int srcPort, int dstPort, string protocolName, string tcpFlags)
        {
            try
            {
                ...
                if (!finalMalicious) // لو signature + behavior ما كشفوش حاجة
                {
                    var aiFeatures = new AiFeatures
                    {
                        DstPort = dstPort,
                        Protocol =(int) ipPacket.Protocol, // حسب نوع الـ Protocol
                        PacketSize = ipPacket.TotalLength,
                        PacketCount = flow.PacketCount,
                        PayloadSize = payload.Length,
                        FlowDirection = MapFlowDirection(direction),
                        TcpFlags = MapTcpFlags(tcpFlags)
                    };

                    try
                    {
                        var aiResult = await _aiEngine.PredictAsync(aiFeatures);
                        if (aiResult != null && aiResult.prediction == 1)
                        {
                            finalMalicious = true;
                            OptimizedLogger.LogImportant($"[AI] AI predicted malicious traffic: {srcIp}:{srcPort} -> {dstIp}:{dstPort}, mse={aiResult.mse}, threshold={aiResult.threshold}");
                        }
                    }
                    catch (Exception ex)
                    {
                        OptimizedLogger.LogError($"[AI-ERROR] Failed to get prediction: {ex.Message}");
                    }
                }
